...
**Next action:** Fable continues the Phase 6 foundations per the approved plan: step 2 — the pure timeline module (app/shared/timeline/: mapping, snap-with-reason, lane packing, previewSchedule) with its tests; then step 3 — the Chromium decoder replacing parseWav in export, the trim-aware mixer, the extended export check (trimmed WAV + M4A clips), OQ-024, and M-5.5's mixing rules. Then the hand-off to Opus for the timeline UI (DOC-11 Appendix G step 10).
