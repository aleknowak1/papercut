...
**Next action:** Phase 6 (Timeline and audio) runs from the kickoff prompt in DOC-11 Appendix G (CL-0053): Fable for the foundations (clip trim fields, pure timeline module, previewSchedule, mixer/decoder, export change, checks), then Opus for the timeline UI; decisions a–n already made. The verification commit (CL-0054) is done; the Fable session's Phase 6 plan is with Alek for approval.
