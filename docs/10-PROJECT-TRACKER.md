...
**Status:** Active
**Last updated:** 2026-09-02 (Phase 4b usability pass from Alek's feedback — per-row scene actions, drag-to-canvas, clear empty states, CL-0041)
...
**Phase 4 — Scene and layers: IN PROGRESS.** Plan approved by Alek (decisions a–i in DOC-11 Appendix E addendum); building alone, one feature at a time. Step 1 done (CL-0037): Layer.hidden/locked and Scene.backgroundFit in the document (old files load unchanged), reorder/hide/lock/fit edits, shared pure geometry, scene check green. Step 2 done (CL-0038): sceneStage is the one drawing path — export renders through it. Step 3 done (CL-0039): editor layout (tabs · canvas · Layers), background + fit, add/order/hide/lock/opacity/flip. Step 4 done (CL-0040): click-select on real pixels, drag with one-commit-on-release and Escape-cancel, uniform corner resize, nudges, Delete. **All Phase 4 features are built, checked, live-verified and in the manual (M-3.1–M-3.3).** Phase 4b (CL-0041, from Alek's feedback): the scene flow reproduced live and made obvious — "Set as background" / "Add to scene" on every relevant row and character card, drag-and-drop from the panels onto the canvas (drop point = placement), and plain guidance instead of disabled controls when a project is empty.
**Next action:** Alek tries Phase 4 by hand (§5), then Phase 5 (Animation) begins — DOC-10 §2 lists Fable for the keyframe engine, then Opus: *"Read CLAUDE.md and DOC-10, then start Phase 5: keyframes (position, scale, rotation, flip, opacity), easing, motion presets, pose swapping, camera pan/zoom, and the render-snapshot check (DOC-01 §5.1, DOC-03 §3)."* Plan-first recommended: the render-snapshot check and the keyframe engine are foundations.
...
| 4 | **Scene and layers** | Background, character/prop layers, ordering, opacity, lock/hide, placing and sizing on canvas | **Complete** (CL-0037–0040: every feature ☑, M-3 sections written, live-verified by driving the real app; Alek's try-out in §5) | Fable |
...
| Place and size on canvas | 4 | ☑ (CL-0040; Alek's own try-out pending, §5) | M-3.3 |
...
| Item | Needed by | Ref |
|------|-----------|-----|
| Try Phase 4 again after the 4b pass: open a project (`npm run dev`), import a background photo, press **Set as background** on its row (or drag the row onto the canvas), import a person, wait for "cutting out…" to finish, press **Add to scene** on the cutout row (or drag it where you want it), then drag/resize/flip, hide/lock/reorder, and Ctrl+Z back through it all. M-3.1–M-3.3 describe it. | Now (verifies Phase 4 + 4b) | M-3.1–M-3.3 |
