// The Web Audio preview (Phase 6 decision i): on play, the clips
// previewSchedule returns are scheduled through one AudioBufferSourceNode
// + GainNode per clip — trim as the source offset and played length,
// volume and fades as the gain envelope. Pause, the scene end, or any
// edit stops sound at once; scrubbing never comes here at all.
//
// previewSchedule is the SAME translation of the document the export
// mixer renders sample-for-sample (CL-0056/0057), and the sounds decode
// through the SAME Chromium door (audioCache.ts) — so what Alek hears on
// play is what exports.
...
/** The audio assets a scene's clips point at, each at most once. */
function sceneAudioAssets(doc: ProjectDocument, scene: Scene): Map<string, string> {
...
/**
 * Starts the audio for a preview playing from `fromTime`. Returns at
 * once; the sound joins as soon as every needed clip is decoded (already
 * instant when the lanes have drawn their waveforms — the cache is
 * shared). Call stop() when the preview pauses or ends.
 */
export function startAudioPreview(
  ...
  const assets = sceneAudioAssets(doc, scene);
  ...
  void Promise.all(decodes).then((entries) => {
    ...
    const schedule = previewSchedule(scene, fromTime + elapsed, durations);
